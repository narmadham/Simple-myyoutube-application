        ...
        if (dates && Object.keys(dates).length) {
            const filteredData = fullData.filter(data => {
                if (
                    dates &&
                    dates.min <
                        new Date(data.snippet.publishedAt).getFullYear() &&
                    new Date(data.snippet.publishedAt).getFullYear() < dates.max
                ) {
                    return data;
                }
            });
...
export const getDates = createSelector([getFullData], fullData => {
    if (fullData.length > 0) {
        let dates = fullData.map(item =>
